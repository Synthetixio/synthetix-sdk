...
class AccountAPI:
    ...
    def get_order_history(
        ...
        triggerPrice (optional string, stop/trigger price for the order), and
        triggerPriceType (optional string, e.g. ``"last_price"`` or ``"mark_price"``).
        """
        kwargs: Dict[str, Any] = {}
        if start_time:
    ...
    def get_balance_updates(
        ...
        fee, collateral, timestamp. ``grossAmount`` is amount plus fee;
        ``fee`` is the fee charged for the balance update. Additional
        fields vary by action: TRANSFER includes ``fromSubAccountId`` /
        ``toSubAccountId``; deposits and withdrawals include
