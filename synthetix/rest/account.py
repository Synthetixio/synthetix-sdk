...
class AccountAPI:
    ...
    def get_positions(
        ...
        Args:
            symbol: Optional market symbol filter (e.g. ``"BTC-USDT"``).
                When set, only positions for the given market are returned.
        ...
        return self._action("getPositions", subaccount_id=subaccount_id, **kwargs)
    ...
    def get_order_history(
        ...
        if limit:
            kwargs["limit"] = limit
        return self._action("getOrderHistory", subaccount_id=subaccount_id, **kwargs)
    ...
    def get_balance_updates(
        ...
        action (``DEPOSIT`` | ``WITHDRAWAL`` | ``TRANSFER``), status
        (``success`` | ``completed`` | ``pending``), amount, grossAmount,
        fee, usdtNotional, collateral, timestamp. ``grossAmount`` is
        amount plus fee; ``fee`` is the fee charged for the balance
        update; ``usdtNotional`` is the spot USDT value at the time
        of the transaction. Additional fields vary by action: TRANSFER
        includes ``fromSubAccountId`` / ``toSubAccountId``; deposits
        and withdrawals include ``destinationAddress`` / ``txHash``.
        """
        kwargs: Dict[str, Any] = {}
        if start_time:
            kwargs["startTime"] = start_time
    ...
    def withdraw_collateral(
        ...
        When signing as a manager/delegated wallet (i.e. not the subaccount
        owner), withdrawals are only permitted to the owner's wallet address;
        any other ``destination`` is rejected with a 403 Forbidden error.
        """
        sid = subaccount_id if subaccount_id is not None else self.subaccount_id
    ...
    def voluntary_collateral_exchange(
        self,
        source_asset: str,
        target_usdt_amount: str,
        subaccount_id: Optional[int] = None,
    ) -> Dict:
        """Voluntarily exchange a non-USDT collateral for USDT.

        Args:
            source_asset: Collateral symbol to exchange from (e.g. ``"BTC"``).
                Cannot be ``"USDT"``.
            target_usdt_amount: Either the literal string ``"all"`` to exchange
                the maximum available, or a positive decimal string of the
                target USDT amount to receive (e.g. ``"100"``).
            subaccount_id: Subaccount ID (defaults to active).

        Returns a dict with:
        - **sourceAsset** — collateral symbol exchanged from
        - **sourceAmountTaken** — decimal string, amount of ``sourceAsset`` consumed
        - **targetAsset** — destination collateral symbol (always ``"USDT"``)
        - **targetAmount** — decimal string, USDT received
        - **indexPrice** — decimal string, index price used for the exchange
        - **effectiveHaircut** — decimal string, haircut applied to the source
        - **collateral** — list of resulting collateral balances, each
          ``{"symbol": str, "quantity": str}``
        """
        sid = subaccount_id if subaccount_id is not None else self.subaccount_id
        request = self._signer.sign_voluntary_collateral_exchange(
            sid, source_asset, target_usdt_amount, expires_after_ms=self.expires_after_ms
        )
        return self._trade(request)

    # ── Fees & Funding ──────────────────────────────────────────────

    def get_fee_rate(self, subaccount_id: Optional[int] = None) -> Dict:
        """Get the current maker/taker fee rate and tier schedule.

        Returns ``{"volume14d": str, "currentTier": {...}, "makerFeeRate": str,
        "takerFeeRate": str, "tiers": [...], "referralDiscountApplied": bool,
        "referralDiscount": str}``.

        ``currentTier`` contains: ``tierId``, ``tierLevel`` (int), ``tierName``,
        ``tierType``, ``minTradeVolume``, ``makerFeeRate``, ``takerFeeRate``.
        Each entry in ``tiers`` contains: ``tierId``, ``tierLevel`` (int),
        ``tierName``, ``minTradeVolume``, ``makerFeeRate``, ``takerFeeRate``.

        ``makerFeeRate``/``takerFeeRate`` include the referral discount when
        ``referralDiscountApplied`` is true; ``referralDiscount`` is the
        multiplier applied (e.g. ``"0.9"``), ``"0"`` when no discount applies.
        """
        return self._action("getFeeRate", subaccount_id=subaccount_id)
    ...
    def get_withdrawable_amounts(
        ...
        """
        return self._action("getWithdrawableAmounts", subaccount_id=subaccount_id, symbols=symbols)
