...
class Signer:
    ...
    def sign_place_orders(
        ...
        # Build param orders: signed fields + unsigned fields (not part of EIP-712 signature)
        param_orders = []
        for orig, fmt in zip(orders, formatted_orders):
            order_params = dict(fmt)
            if orig.get("postOnly"):
                order_params["postOnly"] = True
            if "expiresAt" in orig:
                order_params["expiresAt"] = int(orig["expiresAt"])
            if "durationSeconds" in orig:
                order_params["durationSeconds"] = int(orig["durationSeconds"])
            if "intervalSeconds" in orig:
                order_params["intervalSeconds"] = int(orig["intervalSeconds"])
            param_orders.append(order_params)
        ...
        return self._make_request(
            ...
            "placeOrders",
            subaccount_id,
            {"orders": param_orders, "grouping": grouping},
        )
    ...
    # ── Voluntary Collateral Exchange ──────────────────────────────

    def sign_voluntary_collateral_exchange(
        self,
        subaccount_id: int,
        source_asset: str,
        ...
        """Sign a voluntary-collateral-exchange request."""
        nonce, expires_after = self._nonce_and_expiry(nonce, expires_after_ms)

        message = {
            "subAccountId": subaccount_id,
            "sourceAsset": source_asset,
            "targetUSDTAmount": target_usdt_amount,
            "nonce": nonce,
            "expiresAfter": expires_after,
        }
        ...
        return self._make_request(
            ...
            "voluntaryCollateralExchange",
            subaccount_id,
            {"sourceAsset": source_asset, "targetUSDTAmount": target_usdt_amount},
        )
