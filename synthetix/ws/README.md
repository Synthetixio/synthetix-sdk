...
| Method | Description |
|---|---|
| `await ws_get_positions(symbol=None, subaccount_id=None)` | Open positions (optionally filtered by `symbol`) |
...
| `await ws_get_fee_rate(subaccount_id=None)` | Current maker/taker fee rate and tier schedule |
...
| `await ws_withdraw_collateral(amount, symbol="USDT", destination=None, subaccount_id=None)` | Withdraw collateral |
| `await ws_transfer_collateral(amount, to_subaccount_id, symbol="USDT", subaccount_id=None)` | Transfer between subaccounts |
| `await ws_voluntary_collateral_exchange(source_asset, target_usdt_amount, subaccount_id=None)` | Exchange a non-USDT collateral for USDT |
