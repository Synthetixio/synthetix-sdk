...
class Synthetix:
    ...
    def get_market_prices(self) -> Dict[str, Dict]:
        ...
        timestamp, onHours (optional) — bool, present only for session markets
        when the current within-trading-hours state is known.
        """
    ...
    def get_positions(
        ...
        """
        return self._require_account().get_positions(symbol, subaccount_id)
    ...
    def get_balance_updates(
        ...
        fee, usdtNotional, collateral, timestamp. TRANSFER entries also include
        ``fromSubAccountId`` / ``toSubAccountId``; deposits and withdrawals
        include ``destinationAddress`` / ``txHash``.
        ``grossAmount`` is amount plus fee; ``usdtNotional`` is the spot
        USDT value at the time of the transaction.
        """
    ...
    def voluntary_collateral_exchange(
        ...
        Args:
            source_asset: Collateral symbol to exchange from (e.g. ``"BTC"``).
                Cannot be ``"USDT"``.
            target_usdt_amount: Either ``"all"`` to exchange the maximum
                available, or a positive decimal string of the target USDT
                amount to receive (e.g. ``"100"``).
            subaccount_id: Subaccount ID (defaults to active).

        Returns a dict with: sourceAsset, sourceAmountTaken, targetAsset
        (always ``"USDT"``), targetAmount, indexPrice, effectiveHaircut, and
        ``collateral`` — a list of ``{"symbol": str, "quantity": str}``
        entries describing the resulting collateral balances.
        """
        return self._require_account().voluntary_collateral_exchange(source_asset, target_usdt_amount, subaccount_id)
    ...
    def get_fee_rate(self, subaccount_id: Optional[int] = None) -> Dict:
        ...
        """
        return self._require_account().get_fee_rate(subaccount_id)
    ...
    async def ws_get_positions(
        ...
        return await self._ws_query("getPositions", subaccount_id, **kwargs)
    ...
    async def ws_get_fee_rate(self, subaccount_id: Optional[int] = None) -> Dict:
        """Get the current maker/taker fee rate and tier schedule via WebSocket.
        ...
        multiplier applied (e.g. ``"0.9"``), ``"0"`` when no discount applies.
        """
        return await self._ws_query("getFeeRate", subaccount_id)
    ...
    async def ws_voluntary_collateral_exchange(
        self,
        source_asset: str,
        target_usdt_amount: str,
        subaccount_id: Optional[int] = None,
        ...
        signer, sub_id = self._require_auth()
        sub_id = subaccount_id if subaccount_id is not None else sub_id
        request = signer.sign_voluntary_collateral_exchange(
            sub_id, source_asset, target_usdt_amount, expires_after_ms=self._expires_after_ms
        )
        return await self._ensure_ws().post(request)
