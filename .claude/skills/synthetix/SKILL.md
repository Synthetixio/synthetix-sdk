...
**Account (auth required):** `get_positions()`, `get_open_orders()`, `get_order_history()`, `get_trades()`, `get_trades_for_position(position_id, limit=0, offset=0)`, `get_portfolio()`, `get_balance_updates()`, `get_fees()`, `update_leverage(symbol, leverage)`, `create_subaccount(name)`, `transfer_collateral(amount, to_subaccount_id, symbol)`
...
**WS trading (async):** All REST trading methods have `ws_` prefixed async equivalents — e.g. `await snx.ws_market_order(...)`, `await snx.ws_twap_order(...)`. Also: `await snx.ws_get_exchange_status()` queries both REST and WS servers in parallel. WS queries: `await snx.ws_get_trades_for_position(position_id, limit=0, offset=0)`.
