...
**Trading (auth required):** `market_order(symbol, side, quantity)`, `place_order(symbol, side, quantity, ...)`, `twap_order(symbol, side, quantity, duration_seconds, price=...)`, `modify_order(order_id, ...)`, `cancel_order(order_id)`, `cancel_all_orders(symbol)`
...
**WS trading (async):** All REST trading methods have `ws_` prefixed async equivalents — e.g. `await snx.ws_market_order(...)`, `await snx.ws_twap_order(...)`.
...
- **TWAP** — `twap_order(symbol, side, quantity, duration_seconds=300, price=...)` — time-weighted average price execution over a duration. Optional `price` as a limit ceiling/floor. **Constraints:** minimum notional $10,000 USD (`quantity × markPrice`), duration 300 s (5 min) to 86,400 s (24 h). Executed in 30-second chunks server-side.
...
- `get_open_orders()` — resting orders. Each has `order` (`{"venueId": str}`), `symbol`, `side`, `type`, `price`, `quantity`.
...
- `get_order_history()` — all historical orders (filled, cancelled, rejected). Each has `order` (`{"venueId": str}`), `symbol`, `side`, `type`, `price`, `filledQuantity`, `status`.
