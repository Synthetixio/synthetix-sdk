...
**Trading (auth required):** `market_order(symbol, side, quantity)`, `place_order(symbol, side, quantity, ...)`, `twap_order(symbol, side, quantity, duration_seconds, price=...)`, `modify_order(order_id, ...)`, `cancel_order(order_id)`, `cancel_all_orders(symbol)`, `schedule_cancel(timeout_seconds)` (dead-man switch / auto-cancel)
...
**WS trading (async):** All REST trading methods have `ws_` prefixed async equivalents — e.g. `await snx.ws_market_order(...)`, `await snx.ws_twap_order(...)`. Also: `await snx.ws_get_exchange_status()` queries both REST and WS servers in parallel.
