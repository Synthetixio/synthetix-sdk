...
You are helping a user try out the Synthetix perpetual futures exchange using the `synthetix-sdk` Python package (published on PyPI). Walk them through setup and usage interactively.
...
**Market data (no auth):** `get_markets()`, `get_market_prices()`, `get_orderbook(symbol)`, `get_last_trades(symbol)`, `get_funding_rate(symbol)`, `get_open_interest()`, `get_candles(symbol, interval)`, `get_exchange_status()`, `get_tiers()`
...
**Account (auth required):** `get_positions(symbol=None)`, `get_open_orders()`, `get_order_history()`, `get_trades()`, `get_trades_for_position(position_id, limit=0, offset=0)`, `get_portfolio()`, `get_balance_updates()`, `get_fee_rate()`, `get_withdrawable_amounts(symbols)`, `update_leverage(symbol, leverage)`, `create_subaccount(name)`, `transfer_collateral(amount, to_subaccount_id, symbol)`, `voluntary_collateral_exchange(source_asset, target_usdt_amount)`
...
- `get_tiers()` — exchange fee/account tier configuration. Returns a list of tier dicts, each with `tierId`, `tierType`, `tierName`, `minTradeVolume` (str), `makerFeeRate`/`takerFeeRate` (str), `maxSubAccounts`/`maxOrdersPerMarket`/`maxTotalOrders` (int), `maxBorrowCapacity` (str). Use to display fee schedules or determine which tier the user qualifies for based on volume.

See `examples/rest/fetch-markets.py`, `fetch-ticker.py`, `fetch-orderbook.py`, `fetch-trades.py`, `fetch-open-interest.py`, `fetch-candles.py`, `fetch-exchange-status.py`, `fetch-tiers.py`.
...
- `get_positions(symbol=None)` — open positions. Pass `symbol` (e.g. `"BTC-USDT"`) to filter by market. Each has `symbol`, `side` ("LONG"/"SHORT"), `quantity`, `entryPrice`, `unrealizedPnl`, `liquidationPrice` (optional, use `.get()`).
...
**WS queries:** `await snx.ws_get_positions(symbol=None)`, `await snx.ws_get_open_orders()` — same data, async.
...
- `get_fee_rate()` — your current maker/taker fee rate and tier schedule. Returns `volume14d` (str), `currentTier` (dict: `tierId`, `tierLevel`, `tierName`, `tierType`, `minTradeVolume`, `makerFeeRate`, `takerFeeRate`), top-level `makerFeeRate`/`takerFeeRate` (str), `tiers[]` (full fee schedule), `referralDiscountApplied` (bool), and `referralDiscount` (str multiplier, `"0"` when none). Async: `await snx.ws_get_fee_rate()`.

See `examples/rest/fetch-funding-rate.py`, `fetch-funding-rate-history.py`, `fetch-funding-payments.py`, `fetch-fee-rate.py`, `examples/ws/async-fetch-fee-rate.py`.
...
- `transfer_collateral(amount, to_subaccount_id, symbol="USDT")` — move collateral between subaccounts. Requires at least 2 subaccounts. Convert subaccount ID to int.
- `voluntary_collateral_exchange(source_asset, target_usdt_amount)` — exchange a non-USDT collateral for USDT. `source_asset` is the collateral symbol to convert from (cannot be `"USDT"`); `target_usdt_amount` is either the literal string `"all"` or a positive decimal string of USDT to receive. Returns `{"sourceAsset", "sourceAmountTaken", "targetAsset", "targetAmount", "indexPrice", "effectiveHaircut", "collateral": [{"symbol", "quantity"}]}`. Async: `await snx.ws_voluntary_collateral_exchange(source_asset, target_usdt_amount)`.
- `withdraw_collateral(amount, symbol="USDT", destination=None)` — withdraw to wallet. Defaults to own wallet if destination is omitted. When signing as a manager/delegated wallet (not the subaccount owner), withdrawals are only permitted to the owner's wallet address; any other `destination` returns a 403 Forbidden error.
...
See `examples/rest/create-subaccount.py`, `transfer-collateral.py`, `voluntary-collateral-exchange.py`, `fetch-sub-accounts.py`, `add-delegated-signer.py`, `fetch-delegated-signers.py`, `remove-delegated-signer.py`. WS: `examples/ws/async-voluntary-collateral-exchange.py`.

## Trade history & fills details

- `get_balance_updates(start_time=0, end_time=0, action_filter=None, limit=0, offset=0)` — deposits, withdrawals, and transfers. `action_filter` is a comma-separated list of `DEPOSIT`, `WITHDRAWAL`, `TRANSFER`. `limit` defaults to 50 (max 1000); `offset` defaults to 0 (max 10000). Returns `{"balanceUpdates": [...]}`, each with `id`, `action` (DEPOSIT/WITHDRAWAL/TRANSFER), `status` (success/completed/pending), `amount`, `grossAmount` (amount + fee), `fee`, `usdtNotional` (spot USDT value at the time of the transaction), `collateral`, `timestamp`. TRANSFER entries also include `fromSubAccountId`/`toSubAccountId`; deposits/withdrawals include `destinationAddress`/`txHash`.
