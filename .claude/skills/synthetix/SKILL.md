...
**Market data (no auth):** `get_markets()`, `get_market_prices()`, `get_orderbook(symbol)`, `get_last_trades(symbol)`, `get_funding_rate(symbol)`, `get_open_interest()`, `get_candles(symbol, interval)`

**Trading (auth required):** `market_order(symbol, side, quantity)`, `place_order(symbol, side, quantity, ...)`, `modify_order(order_id, ...)`, `cancel_order(order_id)`, `cancel_all_orders(symbol)`

**Account (auth required):** `get_positions()`, `get_open_orders()`, `get_order_history()`, `get_trades()`, `get_portfolio()`, `get_balance_updates()`, `get_fees()`, `update_leverage(symbol, leverage)`, `create_subaccount(name)`, `transfer_collateral(amount, to_subaccount_id, symbol)`

**WS subscriptions:** All `subscribe()` calls accept an optional `on_error=callback` to handle exceptions without losing the subscription. Examples: `subscribe("marketPrices", callback, on_error=on_error, symbol=...)`, `subscribe("orderbook", callback, on_error=on_error, symbol=..., depth=50, format="snapshot")`, `subscribe("trades", callback, on_error=on_error, symbol=...)`, `subscribe("candles", callback, on_error=on_error, symbol=..., interval=...)`, `subscribe("subAccountUpdates", callback, on_error=on_error)`

**WS trading (async):** All REST trading methods have `ws_` prefixed async equivalents — e.g. `await snx.ws_market_order(...)`.
...
- `get_open_interest()` — OI for all markets. Returns list of dicts with `symbol`, `openInterest`, `longOpenInterest`, `shortOpenInterest`.
...
See `examples/rest/fetch-markets.py`, `fetch-ticker.py`, `fetch-orderbook.py`, `fetch-trades.py`, `fetch-open-interest.py`, `fetch-candles.py`.
...
See `examples/rest/create-market-order.py`, `create-limit-gtc-order.py`, `create-limit-ioc-order.py`, `create-trigger-tp-order.py`, `create-trigger-sl-order.py`, `batch-orders.py`. WS: `examples/ws/async-create-order.py`, `async-market-order.py`.
...
- `get_open_orders()` — resting orders. Each has `orderId`, `symbol`, `side`, `type`, `price`, `quantity`.
...
- `get_trades()` — your executed fills. Response wraps data: use `.get("trades", [])`. Also includes `hasMore` (bool) and `total` (int) for pagination. Each trade has `timestamp`, `symbol`, `side`, `price`, `quantity`, `fee`, `realizedPnl` (optional).
- `get_order_history()` — all historical orders (filled, cancelled, rejected). Each has `orderId`, `symbol`, `side`, `type`, `price`, `filledQuantity`, `status`.
...
See `examples/rest/fetch-my-trades.py`, `fetch-order-history.py`, `fetch-position-history.py`, `fetch-position-history-filtered.py`.
